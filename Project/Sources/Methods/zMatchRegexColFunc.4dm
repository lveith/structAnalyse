//%attributes = {}
  // PM: "zMatchRegexColFunc" (neu LV 21.10.2019)
  ...
  // Z.B.: collection.filter("zMatchRegexColFunc";objItemNameToFilter;regExprFilter) -> newFilteredCol
  // Z.B.: New collection(New object("id";"1:";"name";"A")).filter("zMatchRegexColFunc";objItemNameToFilter;regExprFilter) -> newFilteredCol
  // Z.B.: $newFilteredCol:=$orgCol.filter("zMatchRegexColFunc";$objItemNameToFilter;$regExprFilter)
  ...
  // Vorsicht, würde man diese Methode direkt als PM aufrufen z.B.: zMatchRegexColFunc(obj;objItemNameToFilter;regExprFilter)
  ...
  //    $newFilteredCol:=$orgCol.filter("zMatchRegexColFunc";"id";"\\d+[\\d\\D]*")  // -> [{id:1a,name:A},{id:2b,name:B}] 
  //      // Example 'endsWith' a Digit (RegExpr must fully equal occur with whole value)
  //    $newFilteredCol:=$orgCol.filter("zMatchRegexColFunc";"id";"[\\d\\D]*\\d+")  // -> [{id:c3,name:C}] 
  //      // Example 'contains' a Digit (RegExpr must fully equal occur with whole value)
  //    $newFilteredCol:=$orgCol.filter("zMatchRegexColFunc";"id";"[\\d\\D]*\\d+[\\d\\D]*")  // -> [{id:1a,name:A},{id:2b,name:B},{id:c3,name:C}] 
  ...
  // Letzte Änderung: 21.10.2019, 20:48
...
If ((Error#0) | (OK#0) | (Shift down:C543))
	...
	$1.stop:=True:C214
	ALERT:C41("Fehler im RegExAusdruck, das Filtern wird abgebrochen")
End if 

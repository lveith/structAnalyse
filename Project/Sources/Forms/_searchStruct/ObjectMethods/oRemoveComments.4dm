  // OM: "_searchStruct.oRemoveComments" (new LV 13.07.20, 15:30:03)
  // Last change: LV 13.07.20, 15:30:03
...
Case of 
	: (Form event code:C388=On Clicked:K2:4)
		$vtItems:=""
		$vtItems:=$vtItems+Char:C90(1)+"Remove comments in marked selection ("+String:C10(Form:C1466.soCurrSel.length)+")"
		$vtItems:=$vtItems+Char:C90(1)+"Remove comments in current selection ("+String:C10(Form:C1466.searchResultsDisplay.length)+")"
		$vtItems:=$vtItems+Char:C90(1)+"Remove comments in whole selection ("+String:C10(Form:C1466.searchResults.length)+")"
		If (Length:C16($vtItems)>0)
			$vlUserChoice:=0
			$vlUserChoice:=Pop up menu:C542($vtItems)
			
			Case of 
				: ($vlUserChoice=1)  // Remove comments in marked selection
					$colToUse:=Form:C1466.soCurrSel
					
				: ($vlUserChoice=2)  // Remove comments in current selection
					$colToUse:=Form:C1466.searchResultsDisplay
					
				: ($vlUserChoice=3)  // Remove comments in whole selection
